...
public class Tree {
    ...
    public TNode findNode(String partialKey, int where) {
        if (partialKey.length() == Globals.KEY_LEN) {                           // if full key
            ...
            if (partialKey.compareTo(root.getId().substring(0, partialKeyLen)) < 0) {
                ...
                TNode p = root;      // find the smallest or largest from partialKey depending on 'where'                     
    ...
    public void insertNode(TNode p) {
        insertNodeRec(p);    // one node in tree is a height of 1
        balance(p);
    }

    private void setParentsChildLink(TNode p, TNode q) {
System.out.println("p: " + p + " q: " + q + " p.parent: " + p.getParent());
System.out.println();
...
if (Globals.DEBUG_TREE) {
    if (p == root) { 
        System.out.println("Entered Case 1a");
        Globals.case1a++;
    }
    else {
        System.out.println("Entered Case 1b");
        Globals.case1b++;
    }
}
...
if (Globals.DEBUG_TREE) {
    if (p.getLeft() != null) {
        if (p == root) {
            System.out.println("Entered Case 2a");
            Globals.case2a++;
        }
        else {
            System.out.println("Entered Case 2b");
            Globals.case2b++;
        }
    }
    else {
        if (p == root) {
            System.out.println("Entered Case 3a");
            Globals.case3a++;
        }
        else {
            System.out.println("Entered Case 3b");
            Globals.case3b++;
        }
    }
}
...
if (Globals.DEBUG_TREE) {
    if (p == root) {
        System.out.println("Entered Cases 4a,5a-a (case 4a is a subset of 5a-a)");
        Globals.case5aa++;
    }
    else {
        System.out.println("Entered Case 4b,5a-b (case 4b is a subset of 5a-b)");
        Globals.case5ab++;
    }
}
...
if (Globals.DEBUG_TREE) {
    if (p == root) {
        System.out.println("Entered Cases 5b-a, 5c-a (it might not be necessary to differentiate 5b and 5c)");
        Globals.case5ba++;
    }
    else {
        System.out.println("Entered Cases 5b-b, 5c-b (it might not be necessary to differentiate 5b and 5c)");
        Globals.case5bb++;
    }
}
    ...
    private void updateTreeHeightsOfThisAndBelow() {
        if (this.root != null) {
            int leftHeight = 0;
            if (this.root.getLeft() != null) {
                Tree lTree = new Tree(this.root.getLeft());
                lTree.updateTreeHeightsOfThisAndBelow();
            ...
            if (this.root.getRight() != null) {
                ...
                rTree.updateTreeHeightsOfThisAndBelow();
    ...
    */
                ...
                if (ancestorTree.balanceFactor() == -2) {
                    ...
                    if (rTreeBalanceFactor == -1 || rTreeBalanceFactor == 0) { //0 happens in delete case 7a
                        if (ancestor == root) {
                            ...
                            updateTreeHeightsOfThisAndBelow();
                        }
                        else { 
                            ...
                            ancestorTree.updateTreeHeightsOfThisAndBelow();
    ...
    // this is a modified version of printTree() that prints the level of the node being printed
    ...
    public void printTree(int level) {
        if (root != null) {
            ...
            System.out.println(" in level " + level + " Height: " + root.getHeight());
    ...
    // works based on printTree(TNode, TNode) but sends instead of printing 
